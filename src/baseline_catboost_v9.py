"""Smart warehouse delay prediction — v9 (Multi-Seed Ensemble).
...
def main() -> None:
    ...
    submission_path = project_root / "data" / "submission" / f"submission_v9_{ts}.csv"

    train = pd.read_csv(train_path)
    test = pd.read_csv(test_path)
    layout_info = pd.read_csv(layout_info_path)
    print(f"학습 데이터: {train.shape}  테스트 데이터: {test.shape}  레이아웃: {layout_info.shape}")
    print(f"log transform: {USE_LOG_TRANSFORM} (v9 default=False)")
