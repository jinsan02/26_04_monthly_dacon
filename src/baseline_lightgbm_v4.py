"""Smart warehouse delay prediction — v4 (일반화 특화, log_transform=True).
...
V4 추가 사항:
    - HRI 지표: staff_on_floor / robot_active, worker_avg_tenure_months 기반 상호작용
    - 통신/환경 지표: network_latency_ms, wifi_signal_db, floor_vibration_idx, ambient_noise_db 결합
...
def monitor_focus_feature_importance(
    ...
    """v4 핵심 피처(HRI/통신/환경)의 중요도 순위를 출력한다."""
    ...
    print(f"\n[v4-check] 핵심 피처 Top{top_n} 진입: {len(in_top)}/{len(present_focus)}")
    if in_top:
        for k in in_top:
    ...
    if out_top:
        print("[v4-check] Top20 미진입 피처(상호작용 강화 후보):")
        print("  " + ", ".join(out_top))
...
def main() -> None:
    ...
    importance_path = project_root / "reports" / "eda" / "feature_importance_v4.csv"
    ...
    # v4 최종 체크리스트: HRI/통신/환경 피처 Top20 진입 여부 모니터링
    monitor_focus_feature_importance(importance, top_n=20)
    ...
    print("\n시각화: python src/report_importance.py --csv reports/eda/feature_importance_v4.csv")
