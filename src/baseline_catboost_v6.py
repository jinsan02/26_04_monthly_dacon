"""Smart warehouse delay prediction — v6 (Simplify & Interaction).

핵심 전략:
1) 단일 모델: CatBoostRegressor
2) 피처 다이어트: 1차 학습 중요도 기준 Top-N 피처 재학습
3) 비선형 상호작용: exp(congestion_score / 100) * sensor_noise_stress
4) 앙상블 없이 submission_v6 단일 출력
"""
...
def main() -> None:
    ...
    submission_path = project_root / "data" / "submission" / f"submission_v6_{ts}.csv"

    train = pd.read_csv(train_path)
    test = pd.read_csv(test_path)
    layout_info = pd.read_csv(layout_info_path)
    print(f"학습 데이터: {train.shape}  테스트 데이터: {test.shape}  레이아웃: {layout_info.shape}")
    print(f"log transform: {USE_LOG_TRANSFORM} (v6 default=True)")
    ...
    # 2차 학습: Top-N 피처 다이어트 후 재학습
    top_features = select_top_features(feature_cols, importance, top_n=TOP_N_FEATURES)
